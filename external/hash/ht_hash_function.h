...
/* Avoid 0 special case in hash functions and allow for configuration with
 * unguessable seed. */
#ifndef HT_HASH_SEED
...
/* This assumes the key points to a 32-bit aligned random value that is its own
 * hash function. */
static inline size_t ht_uint32_identity_hash_function(const void *key,
                                                      size_t len) {
  (void)len;
  return (size_t) * (uint32_t *)key;
}

/* This assumes the key points to a 64-bit aligned random value that is its own
 * hash function. */
static inline size_t ht_uint64_identity_hash_function(const void *key,
                                                      size_t len) {
  (void)len;
...
static inline uint32_t ht_fnv1a32_str_hash_function(const void *key,
                                                    size_t len) {
#ifndef FNV1A_NOMUL
  const uint32_t prime = UINT32_C(0x1000193);
#endif
...
/* Hashes until string termination and ignores length argument. */
static inline uint64_t ht_fnv1a64_str_hash_function(const void *key,
                                                    size_t len) {
#ifndef FNV1A_NOMUL
  const uint64_t prime = UINT64_C(0x100000001b3);
#endif
